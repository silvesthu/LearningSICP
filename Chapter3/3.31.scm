...
; first. probe show nothing when first set
; second. half-addr is initialized but action is not called -> after-delay is not called
; thrid. propagate, but nothing scheduled, nothing happened
; fourth. after the second propagate, the second part of half-addr (INVERT) finally get chance to be executed
; fifth. the third part (second AND) never be able to run, so sum is never updated
