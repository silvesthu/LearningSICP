...
(define (div-series s1 s2)
	(let ((denom (stream-first s2)))
		(if (eq? denom 0)
			(error "denominator with zero constant term")
			(scale-stream (mul-series s1 (invert-unit-series (scale-stream s2 (/ 1 denom)))) (/ 1 denom))
		)
	)
)
...
; checked with http://mathworld.wolfram.com/MaclaurinSeries.html
